	...
	public VentasAlta_view() {
		...
		comboBox_vuelo.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				...
				for (int i = 0; i < listVuelos.size(); i++) {
					if (listVuelos.get(i).getNumVuelo().equals(numeroVuelo)) {

						for (Aerolinea a : listAerolineas) {
							if (a.getId() == listVuelos.get(i).getAerolinea().getId()) {
								textField_aerolinea.setText(a.getNombre());
